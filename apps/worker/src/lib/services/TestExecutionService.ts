...
import { CodeExecutionRequest } from "../interfaces/CodeExecution";
import { ITestCase, ITestDocument } from "../interfaces/Test";
import { CodeExecutionService } from "./CodeExecutionService";
import { CodeSubmissionResult } from "../../types/queue.types";
...
export class TestExecutionService {
    ...
    /**
     * NEW: Execute code submission using test data from job payload (NO DATABASE ACCESS)
     * This method is used by the worker to execute code without connecting to MongoDB
     */
    async executeWithTestData(
        ...
        try {
            logger.info('Executing code with test data from job payload (no database)', 'TestExecutionService');

            const testResults: TestExecutionResult[] = [];
            let totalExecutionTime = 0;
            let passedTestCases = 0;

            // Execute code against each test case from job data
            for (const testCase of testData.testCases) {
                try {
                    // Convert test data format to ITestCase format
                    const testCaseData: ITestCase = {
                        _id: testCase._id,
                    ...
                    // Create minimal test document structure
                    const testDocument: Partial<ITestDocument> = {
                        executeCodeName: testData.executeCodeName,
                        constraints: testData.constraints,
                        testCases: [testCaseData] as any
                    };
                    ...
                    logger.error(`Error executing test case ${testCase._id}:`, 'TestExecutionService', {
            ...
            const score = testData.testCases.length > 0
                ? (passedTestCases / testData.testCases.length) * 100
                : 0;

            logger.info(`Execution completed: ${passedTestCases}/${testData.testCases.length} test cases passed`, 'TestExecutionService');

            // Return result WITHOUT saving to database (API will handle saving)
            return {
                submissionId,
                testId,
                userId,
            ...
            logger.error('Error executing test submission:', 'TestExecutionService', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
...
int main() {
 ...
 console.log("==================================");
         console.log("First 200 chars:", wrappedCode.substring(0, 200));
            ...
            if (result.status.id >= 5) {
                ...
                testResult.errorMessage = 'Output mismatch: Actual output did not match expected output.';
            }
            return testResult;

        } catch (error) {
            logger.error('Error executing test case:', 'TestExecutionService', { error: error instanceof Error ? error.message : 'Unknown error' });
            return {
                testCaseId: testCase._id || '',
                passed: false,
                expectedOutput: testCase.expectedOutput,
